...
export default function LiveMap() {
  ...
  // Derive routePath and calcDistance to avoid synchronous setState in effect
  const routePath = useMemo(() => {
    if (!pickupCoords || !destCoords) return [];
    if (fetchedRoute.length > 0) return fetchedRoute;
    return [pickupCoords, destCoords]; // Initial straight line
  ...
  useEffect(() => {
    if (!pickupCoords || !destCoords) {
      queueMicrotask(() => {
        if (fetchedRoute.length > 0) setFetchedRoute([]);
        if (fetchedDistance !== 0) setFetchedDistance(0);
      });
      setDistance(0);
      return;
    }
    ...
    const fetchRoute = async () => {
      try {
        ...
        setFetchedDistance(haversineDist);
        setDistance(haversineDist);
      }
    };
    
    fetchRoute();
  }, [pickupCoords, destCoords, setDistance, fetchedRoute.length, fetchedDistance]);
